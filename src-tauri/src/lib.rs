...
fn open_official_fallback(app: tauri::AppHandle, token: String) -> Result<(), String> {
    ...
    // 只注入 token：官网 index.min.js 初始化时若 sessionStorage['token'] 存在即视为已登录
    // 并拉取学生信息。务必不要写 currentBatch——官网把它当 JSON 对象 JSON.parse，写入裸
    // 字符串会抛 JSON 解析错误、打断初始化（实测过）。serde_json 保证 token 作为安全的
    // JS 字符串字面量嵌入（防注入）。
    let token_js = serde_json::to_string(&token).map_err(|e| e.to_string())?;
    let script = format!(
        "try{{sessionStorage.setItem('token',{token_js});}}\
         catch(e){{console.error('[funky-lesson] inject error',e);}}"
    );
